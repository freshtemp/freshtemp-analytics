...
$week_end_day = "2018-07-22";

$c1 = $dbMaster->prepare("DELETE FROM weekly_location_task_metrics WHERE organization = ? AND week_end_day = ?");

$c1->execute(array($organization, $week_end_day));

$c1 = $dbMaster->prepare("DELETE FROM weekly_division_task_metrics WHERE organization = ? AND week_end_day = ?");

$c1->execute(array($organization, $week_end_day));

$c1 = $dbMaster->prepare("DELETE FROM weekly_district_task_metrics WHERE organization = ? AND week_end_day = ?");

$c1->execute(array($organization, $week_end_day));

//Only check for tasks marked null.
$sth = $dbMaster->prepare("SELECT id,name,timezone,day_week_starts FROM organization WHERE id = ?");

$sth->execute(array($organization));
...
while ($row = $sth->fetch()) {
  ...
  foreach($Locations AS $Location) {
    ...
    generateLocationMetrics($dbSlave, $dbMaster, $Location, $week_end_day, 1);
  ...
  foreach($Divisions AS $Division) {
    ...
    generateDivisionMetrics($dbSlave, $dbMaster, $Division, $week_end_day, 1);
  ...
  foreach($Districts AS $District) {
    ...
    generateDistrictMetrics($dbSlave, $dbMaster, $District, $week_end_day, 1);
...
function generateLocationMetrics($dbSlave, $dbMaster, $Location, $nextReportWeekEndDay, $numberOfReportsToGenerate) {
  ...
  $c1 = $dbMaster->prepare("DELETE FROM weekly_location_task_group_metrics WHERE location = ? AND week_end_day = ?");

  $c1->execute(array($Location->id, $reportWeekEndDay));
...
function calculateMetricsForDistrictBetweenTwoDays($dbSlave, $dbMaster, $District, $reportWeekStartDay, $reportWeekEndDay) {
  ...
  if($totalLocations > 0) {
    ...
    if($Results) {
      ...
      $totalCompletedPlusMissed = $completedScheduledTasks + $missedTasks;
