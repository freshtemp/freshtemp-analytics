...
$sth->execute(array($organizationID));
...
while ($row = $sth->fetch()) {
  ...
  $nextReportWeekEndDay = "2019-02-17";

  $Locations = getLocationsFromOrganization($dbSlave, $OrganizationId);

  foreach($Locations AS $Location) {

    echo $Location->name . "-" . $Location->MaxWeekEndDay . "\n";

    generateLocationMetrics($dbSlave, $dbMaster, $Location, $nextReportWeekEndDay, 1);
...
function get_summary_from_single_location_between_two_dates($db, $locationId, $reportWeekStartDay, $reportWeekEndDay) {
  ...
  echo "Start Day: " . $reportWeekStartDay . "\n";
  echo "End Day: " . $reportWeekEndDay . "\n";
